...
defineOptions({ name: 'ScheduleCalendarSearch' });
...
const model = defineModel<Api.Scheduling.ScheduleCalendarSearchParams>('model', { required: true });
...
<template>
  <ElCard class="card-wrapper">
    <ElCollapse>
      <ElCollapseItem :title="$t('common.search')" name="calendar-search">
        <ElForm ref="formRef" :model="model" label-position="right" :label-width="80">
          <ElRow :gutter="24">
            ...
            <ElCol :lg="6" :md="8" :sm="12">
              <ElFormItem label="班组">
                <ElSelect v-model="model.teamId" clearable placeholder="请选择班组">
                  <slot name="teams" />
                </ElSelect>
              </ElFormItem>
            </ElCol>
