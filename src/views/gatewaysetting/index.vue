...
import { fetchGatewaySettingList, fetchDeleteGatewaySetting, fetchBatchDeleteGatewaySetting } from '@/service/api/gatewaysetting';
...
const { columns, columnChecks, data, getData, getDataByPage, loading, mobilePagination, selectedRowKeys } = useUIPaginatedTable({
  ...
  columns: () => [
    ...
    {
      prop: 'runStatus', label: '运行状态', minWidth: 80,
      formatter: row => {
...
<template>
  <div class="min-h-500px flex-col-stretch gap-16px overflow-hidden">
    <ElCard class="card-wrapper">
      <ElForm :model="searchParams" label-width="80px" class="flex flex-wrap gap-16px">
        <ElFormItem label="网关编号" class="w-280px">
          <ElInput v-model="searchParams.gatewayNum" placeholder="搜索网关编号" clearable @keyup.enter="getDataByPage" />
        </ElFormItem>
        <ElFormItem label="网关名称" class="w-280px">
          <ElInput v-model="searchParams.gatewayName" placeholder="搜索网关名称" clearable @keyup.enter="getDataByPage" />
        </ElFormItem>
        ...
        <ElFormItem class="ml-auto">
          ...
          <ElButton @click="() => { searchParams.gatewayNum = undefined; searchParams.gatewayName = undefined; searchParams.runStatus = undefined; getDataByPage(); }">重置</ElButton>
        </ElFormItem>
      </ElForm>
    </ElCard>
    <ElCard class="card-wrapper sm:flex-1-hidden">
      ...
      <ElTable v-loading="loading" height="100%" border :data="data" row-key="id" @selection-change="(rows: any[]) => selectedRowKeys = rows.map(r => r.id)">
      ...
      <GatewaySettingDrawer v-model:visible="drawerVisible" :operate-type="operateType" :row-data="editingData" @submitted="getDataByPage" />
    </ElCard>
  </div>
</template>